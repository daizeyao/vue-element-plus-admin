...
async function encode(req, res) {
  ...
  // const encodePath = os.platform() === 'win32'
  //   ? path.join(__dirname, '../cpp/AES_CBC.exe')
  //   : path.join(__dirname, '../cpp/AES_CBC')
  const encodePath = os.platform() === 'win32' ? './cpp/AES_CBC.exe' : './cpp/AES_CBC'
  ...
  if (files.length > 0) {
    ...
    const command =
      os.platform() === 'win32'
        ? `${encodePath} 1 ${key} ${filePath}`
        : `./${encodePath} 1 ${key} ${filePath}`
    exec(command, (error, stdout, stderr) => {
      if (error) {
        // console.error(`执行加密出错: ${error}`)
        res.set('Content-Type', 'application/json') // 设置Content-Type为application/json
        res.send({ code: 500, message: `执行加密出错: ${error}` })
        return
      }
      if (stderr) {
        res.set('Content-Type', 'application/json') // 设置Content-Type为application/json
        res.send({ code: 500, message: `执行加密出错: ${stderr}` })
        return
      }
