  ...
  for (const key in config.answers) {
    ...
    if (ansInfo instanceof Array) {
      ...
      if (stdAns instanceof Array) {
        ...
        if (stdAns.length === ans.size && Set.isSuperset(stdSet, ans))
          report(STATUS.STATUS_ACCEPTED, fullScore, 'Correct')
        else if (ans.size && Set.isSuperset(stdSet, ans))
          report(STATUS.STATUS_WRONG_ANSWER, Math.floor(fullScore / 2), 'Partially Correct')
